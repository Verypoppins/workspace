...
//Por ejemplo, dado el array de entrada nums=[19, 10, 3, 7, 9, 8, 5, 2, 1, 17, 16, 14, 12, 18, 6, 13, 11, 20, 4, 15]
...
//Creo primero la clase
class SuperArray extends Array {
  snail(rowsCount, colsCount) {
    //Genero un if que compruebe si la entrada es válida y sino tiene que devolver un array vacío
    if (rowsCount * colsCount !== this.length) {
      return [];
      //Aquí creamos la matriz bidimensional
      const matriz = Array.from({ length: rowsCount }, () => Array(colsCount));
      let index = 0;

      for (let col = 0; col < colsCount; col++) {
        if (col % 2 === 0) {
          // Rellenar de arriba hacia abajo en columnas pares
          for (let row = 0; row < rowsCount; row++) {
            matriz[row][col] = this[index++];
          }
        } else {
          // Rellenar de abajo hacia arriba en columnas impares
